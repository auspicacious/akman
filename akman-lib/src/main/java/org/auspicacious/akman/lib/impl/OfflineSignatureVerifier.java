...
public class OfflineSignatureVerifier {
    ...
    public boolean verifyCMS(CMSSignedData s) throws Exception {
        ...
        // or something else? I don't see where the content is
        // verified.
        ...
        for (SignerInformation signer : signerInfoStore) {
            ...
            Collection<X509CertificateHolder> matches = signerCerts.getMatches(selector);
            Iterator<X509CertificateHolder> iterator = matches.iterator();
            X509CertificateHolder cert = iterator.next();
